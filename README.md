...
- Renders email template folders with a language switcher UI (see below)
- Watches the template, all its includes/extends/imports, and your mock data for changes
- Automatically reloads the browser the moment any watched file is saved
...
### Render an email template folder

Point the server at a folder that contains language sub-directories, each with `html.njk` and `subject.njk`:
...
```bash
node server.js /path/to/my-email
```

The browser shows a shell UI with:

- **Language tabs** at the top — click to switch between `NL`, `EN`, etc.
- **Subject line** — the rendered output of `subject.njk`
- **Email preview** — the rendered output of `html.njk` in an isolated iframe

Hot reload works for all languages simultaneously. The current language is also available inside templates as `{{ lang }}`.

If the folder doesn't match the expected structure, the server exits with a clear error.
...
No WebSocket, no build pipeline.

### What triggers a reload

| File changed | Effect |
|---|---|
| The template (or any `html.njk` / `subject.njk` in email-folder mode) | Reload |
...
- **Access the active language in templates** — in email-folder mode, `{{ lang }}` contains the current language code (e.g. `"nl"` or `"en"`).
